...
**Addition of 15 September 2026.** An entry has a screen of its own. The list tile was doing two jobs — summary and editor — and the editor half was a textarea wedged into a row, which is why nothing beyond `raw` was ever editable there. Tapping a tile now opens a full-screen view: the note, the labels the model produced, the details, and the enrichment controls. The consequence that matters is for invariant 1: `userEdited` has recorded hand-edited fields since v0 and nothing could write to it, so "a hand-edited field is never re-derived" was a promise the code made and the UI could not test. The label editors are the first writers, and a test now drives them rather than the seed hook. `raw` stays outside that list deliberately — it records fields the model fills, and the user's own sentence is not one.

**Addition of 12 September 2026.** The self-test answers "does the model work on this device"; it does not answer "why did *this* entry come out like that". That second question needs the conditions the individual pass ran under, and those were being computed and thrown away — three round trips in September were spent reconstructing, from symptoms, whether the strict output grammar had been in force. Each pass now records `enrichment.grammar` and `enrichment.parse` on the record alongside the model and timings, so a result carries its own provenance and an old entry reports the state it actually ran under rather than today's settings. Settings can render them under each entry, with a one-tap copy that pairs the run with the device diagnostics. `parse` is the quality signal the system had been discarding: a result salvaged from broken JSON is a marginal model wearing a success, and was previously indistinguishable from a clean one.
